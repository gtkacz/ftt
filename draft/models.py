...
class Draft(models.Model):
	...
	def print_picks(self, *, recent_only: bool = False):
		output = []
		start_at_round = (
			1 if not recent_only else self.draft_positions.filter(is_current=True).first().pick.round_number - 1
		)

		for i in range(
			DraftPick.objects.filter(draft=self, pick__round_number=start_at_round).first().overall_pick,
			1000,
		):
...
class DraftPick(models.Model):
	...
	selected_player = models.ForeignKey(Player, on_delete=models.SET_NULL, null=True, blank=True)
	...
	def generate_contract(self) -> Contract:
		...
		if self.draft.is_league_draft:
			...
			elif self.pick.round_number == 8 or self.pick.round_number == 9 or self.pick.round_number == 10:
				data = {
					"duration": 1,
					"salary": 5,
					"is_to": True,
			...
			elif self.pick.round_number == 11 or self.pick.round_number == 12 or self.pick.round_number == 13:
				data = {
					"duration": 2,
					"salary": 3.5,
				}
	...
	def make_pick(self, player: Player | None, *, is_auto_pick: bool = False) -> Player:
		...
		with transaction.atomic():
			...
			if next_pick:
				...
				queue = DraftQueue.objects.filter(team=next_pick.pick.current_team, draft=self.draft).first()
...
class DraftQueue(models.Model):
	...
	team = models.ForeignKey(Team, on_delete=models.CASCADE, related_name="draft_queues")
